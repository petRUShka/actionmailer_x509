...
namespace :actionmailer_x509 do
  ...
  task(:send_test => :environment) do
    if ENV['email'].nil?
      ...
      Notifier.deliver_fufusigned("<#{ENV['email']}>", "<demo@foobar.com>", "Signed mail at #{Time.now.to_s}")
    end
  end
  ...
  task(:performance_test => :environment) do
    ...
    Benchmark.bm do |x|
      x.report("#{n} mails without signature: ") {
        for i in 1..n do
        Notifier.create_fufu("<destination@foobar.com>", "<demo@foobar.com>")
        end
      }
      x.report("#{n} mails with signature: ") {
        for i in 1..n do
        Notifier.create_fufusigned("<destination@foobar.com>", "<demo@foobar.com>")
        end
      }
    end
  end
